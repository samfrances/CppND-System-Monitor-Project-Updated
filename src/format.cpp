...
string Format::ElapsedTime(long seconds) {
  ...
  const string minutes_str = std::to_string(minutes);
  const string seconds_str = std::to_string(seconds);

  const int n_zero = 2;
  const auto sep = ':';

  const string hours_trail = std::string(n_zero - hours_str.length(), '0');
  const string minutes_trail =
      std::string(n_zero - minutes_str.length(), '0');
  const string seconds_trail =
